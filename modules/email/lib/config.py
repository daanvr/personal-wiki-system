"""Configuration helpers for the email block.

Reads the simple ``KEY=value`` files used across this repo (the root
``config``, provider profiles, and this module's account ``config``),
resolves the knowledge base location, and looks up the account secret
without ever storing it in a committed file.
"""
...
# .../system/modules/email/lib/config.py
#   parents[1] == .../modules/email   (this module)
#   parents[3] == .../system          (repo root)
